...
type Tx = {
  [M in
    ...
    | "fosterLog" | "kindlingLog" | "pregnancyTestLog" | "litter"
    | "breeding" | "rabbit" | "breed"]: { deleteMany(): Promise<unknown> };
};

/**
 * Deletes every farm-data row of the ACTIVE farm, in FK-safe order (Restrict
 * relations — FosterLog/KindlingLog/PregnancyTestLog/Breeding all reference
 * Rabbit with Restrict, so they must go before Rabbit itself; Litter must go
 * before Breeding for the same reason). The farm scoping is invisible here:
