import type { Dictionary } from "./ar";

export const en: Dictionary = {
  nav: {
    dashboard: "Dashboard",
    daily: "Daily Log",
    herdAndStock: "Flock & Stock",
    bucksFertility: "Bucks Fertility",
    mating: "Mating",
    pregnancyTest: "Pregnancy Tests",
    nestBox: "Nest Boxes",
    kindling: "Kindling",
    fostering: "Fostering",
    weaning: "Weaning",
    weaningSales: "Sales",
    does: "Farm Operations",
    dailyRounds: "Doe & Buck Rounds",
    health: "Health",
    reports: "Reports",
    records: "Records",
    operations: "Daily Operations",
    supportOps: "Support Operations",
    finance: "Finance",
    settings: "Settings",
    tagline: "Every litter starts with good care 🐇",
    openMenu: "Open menu",
    closeMenu: "Close menu",
    toggleLabel: "العربية",
    searchPlaceholder: "Search by tag, cage, or breed…",
    searchAriaLabel: "Search for a rabbit",
    searchNoResults: "No results",
  },
  common: {
    selectPlaceholder: "Select…",
    none: "None",
    saving: "Saving…",
    weightUnitKg: "kg",
    weightUnitLb: "lb",
    weightUnitOz: "oz",
    weightChartLabel: "Weight",
    noWeightRecords: "No weight records yet.",
    appDescription: "Rabbit breeding farm management",
    exportExcel: "Export to Excel",
    exporting: "Exporting…",
    exportFailed: "Couldn't export the file",
  },
  settings: {
    title: "Settings",
    description: "Farm-wide preferences for units, breeding, and currency.",
    weightUnitLabel: "Weight unit",
    weightUnitHint: "How weights are entered and displayed.",
    currencyLabel: "Currency code",
    currencyHint: "A 3-letter ISO code, e.g. EGP, USD, or EUR.",
    gestationDaysLabel: "Gestation length (days)",
    gestationDaysHint: "Used to calculate expected kindling dates (default 30).",
    gestationWindowDaysLabel: "Kindling window (± days)",
    gestationWindowDaysHint:
      "How many days early/late still count as 'due' rather than 'overdue'.",
    pregnancyTestDaysLabel: "Pregnancy test wait (days)",
    pregnancyTestDaysHint:
      "Days after mating before a doe appears on the 'Pregnancy Tests' page (default 10).",
    palpationCheckDaysLabel: "Palpation re-check wait (days)",
    palpationCheckDaysHint:
      "Days after mating before the 'Confirm Palpation' button (resorption check) appears (default 15).",
    weaningDaysLabel: "Weaning wait (days)",
    // {max} moves with the rebreed interval — see the note in ar.ts.
    weaningDaysHint:
      "Days after kindling before a doe appears on the 'Weaning' page (max {max}).",
    nestBoxDaysLabel: "Nest box setup wait (days)",
    nestBoxDaysHint:
      "Days after mating before a doe appears on the 'Nest Boxes' page (default 27).",
    matingWeightGramsLabel: "Mating weight (grams)",
    matingWeightGramsHint:
      "Reference only for now — adding it to the does/bucks table is done manually from the add-rabbit page.",
    rebreedLabel: "Rebreed after kindling (days)",
    rebreedHint:
      "How long before a nursing doe becomes ready to mate again and shows up under “Does ready to mate”.",
    rebreedIntensive: "Intensive",
    rebreedSemiIntensive: "Semi-intensive",
    rebreedNatural: "Natural",
    // {placeholder} template, not a function — see the note in ar.ts.
    rebreedCyclesBadge: "{cycles} cycles a year on average",
    fosterWindowDaysLabel: "Fostering lookback (days)",
    fosterWindowDaysHint:
      "Only does that kindled within this many days show up in the two fostering lists (default 2).",
    fosterHighKitsLabel: "Large litter threshold (kits)",
    fosterHighKitsHint:
      "A doe nursing this many kits or more is listed as a donor (default 8).",
    fosterLowKitsLabel: "Small litter threshold (kits)",
    fosterLowKitsHint:
      "A doe nursing this many kits or fewer is listed as a recipient (default 4).",
    pricingSectionTitle: "Prices & costs",
    pricingSectionHint:
      "Reference figures for this farm — they pre-fill forms and compute costs, and never post a transaction on their own. Leave blank to skip.",
    defaultPricePerKgLabel: "Meat price per kg",
    defaultPricePerKgHint:
      "Pre-fills the weaned-kit sale form; still editable on every sale.",
    feedPricePerTonLabel: "Feed price per ton",
    feedPricePerTonHint: "Turns a quantity in tons into an amount on a feed expense.",
    rationsSectionTitle: "Daily feed rations (g/head)",
    rationsSectionHint:
      "A rabbit eats by what it is doing, not by what it is — an empty, a pregnant and a nursing doe are three different numbers. These drive the expected feed load and cost for the herd as it stands.",
    rationDoeIdleLabel: "Empty doe",
    rationDoePregnantLabel: "Pregnant doe",
    rationDoeNursingLabel: "Nursing doe (with her litter)",
    rationBuckLabel: "Buck",
    rationGrowerLabel: "Grower (post-weaning)",
    rationJuvenileLabel: "Replacement stock",
    feedPlanTitle: "Expected feed for the current herd",
    feedPlanCompositionLabel: "Herd composition",
    feedPlanKgPerMonthLabel: "Feed per month",
    feedPlanCostPerMonthLabel: "Feed cost per month",
    feedCostPerDoePerDayLabel: "Feed cost per doe per day",
    feedPlanNote:
      "Expected, computed from the rations and today's head count — not from actual feed bills. Compare it against the feed expense on the finance page: a wide gap means overfeeding, waste, or rations entered wrong.",
    feedPlanEmpty: "Enter the rations and the feed price per ton to see the expected figure.",
    feedPlanHeadUnit: "head",
    saveButton: "Save settings",
    savedToast: "Saved",
    breedAlreadyExists: "This breed is already registered",
    breedAdded: "Breed added",
    breedsHeading: "Rabbit breeds",
    breedsDescription: "Breeds registered here appear as a dropdown when adding or editing a rabbit.",
    noBreeds: "No breeds registered yet.",
    breedDeletedToast: "Breed deleted",
    newBreedLabel: "New breed",
    newBreedPlaceholder: "e.g. New Zealand White",
    addBreedButton: "Add",
    dangerZoneHeading: "Danger zone",
    resetOpsDescription:
      "Permanently deletes every recorded operation on the farm — matings, pregnancy tests, kindlings, weanings, fostering, rabbit weights, health records, financial transactions, stock movements, and deaths/cullings — from all devices.",
    resetOpsKeepNote:
      "Kept: living rabbits, bucks, and registered breeds. Rabbits recorded as deceased or culled are deleted, and every doe's state is reset to “empty”.",
    resetOpsConfirmWord: "RESET OPERATIONS",
    resetOpsConfirmLabel: "Type “{word}” to confirm",
    resetOpsButton: "Delete all operations",
    resettingOpsLabel: "Deleting…",
    resetOpsSuccessToast: "All operations deleted",
  },
  // Mobile-only (offline app) Danger Zone strings. Kept out of `settings`
  // above because that section is passed wholesale from a Server Component
  // to web Client Components — resetConfirm is a function, and RSC cannot
  // serialize functions across that boundary.
  syncRejected: {
    title: "Rejected operations",
    subtitle:
      "Edits that looked saved on this device but the server refused, so they exist here and nowhere else. Review each one: send it again once the cause is fixed, or discard it for good.",
    empty: "Nothing rejected — everything you recorded reached the server.",
    badge: (n: number) => (n === 1 ? "1 rejected" : `${n} rejected`),
    open: "Review",
    colOperation: "Operation",
    colWhen: "Recorded",
    colReason: "Why it was refused",
    retry: "Send again",
    dismiss: "Discard",
    dismissAll: "Discard all",
    dismissConfirm: "This edit will be deleted for good and will never reach the farm. Sure?",
    dismissAllConfirm: "Every rejected edit will be deleted for good. Sure?",
    retryQueued: "Queued again — it will go out with the next sync",
    unknownReason: "Refused without a stated reason",
    rawReason: (code: string) => `Server said: ${code}`,
    reasons: {
      WEANED_EXCEEDS_BORN_ALIVE: "Weaned is larger than the number born alive",
      NO_LITTER: "No litter recorded for this breeding",
      BLANK_REFERENCE: "The row isn't linked to a record",
      TAG_IN_USE: "That ear tag is already in use",
      NO_CURRENT_LITTER: "The doe has no current litter",
      NO_NURSING_KITS: "No live kits to record a death for",
      CAGE_REQUIRED: "A cage must be chosen",
      LITTER_ALREADY_EXISTS: "This breeding already has a kindling",
    } as Record<string, string>,
    opTypes: {
      startBreeding: "Start breeding",
      setBreedingOutcome: "Breeding outcome",
      setPregnancyTestResult: "Palpation result",
      markMated: "Record mating",
      confirmPregnant: "Confirm pregnancy",
      confirmPalpation: "Confirm palpation",
      confirmResorption: "Record resorption",
      installNestBox: "Install nest box",
      markKindled: "Record kindling",
      markWeaned: "Record weaning",
      setLitterCount: "Edit litter counts",
      setLitterWeaningWeight: "Weaning weight",
      recordNursingKitDeath: "Nursing kit death",
      markMatingFailed: "Failed mating",
      clearDoeRow: "Clear doe row",
      setMatingDate: "Edit mating date",
      recordKindling: "Record kindling",
      setDoeState: "Change doe state",
      setRabbitStatus: "Change rabbit status",
      updateRabbitDetails: "Edit rabbit details",
      createQuickRabbit: "Add rabbit",
      finalizeMother: "Promote to doe",
      finalizeBuck: "Promote to buck",
      deleteRabbit: "Delete rabbit",
      saveQuickRabbitCage: "Set cage",
      saveQuickRabbitWeight: "Record weight",
      promoteToHerdPen: "Promote to herd pen",
      transferKits: "Foster kits",
      recordKitSale: "Kit sale",
      recordWeanedKitDeath: "Weaned kit death",
      recordKitStockAdjustment: "Stock adjustment",
      deleteKitStockMovement: "Delete stock movement",
      createHealthRecord: "Health record",
      deleteHealthRecord: "Delete health record",
      createTransaction: "Finance entry",
      deleteTransaction: "Delete finance entry",
      updateRecurringExpenses: "Recurring expenses",
      postRecurringExpenses: "Post recurring expenses",
      updateSettings: "Update settings",
      addBreed: "Add breed",
      deleteBreed: "Delete breed",
    } as Record<string, string>,
  },
  auth: {
    missingFields: "Enter your email and password",
    noFarmMembership: "Your account is not a member of any farm — contact the farm owner",
    switchFarmLabel: "Switch farm",
    loginSubtitle: "Sign in to sync your farm's data",
    registerSubtitle: "Create an account to manage your farm",
    loginTab: "Sign in",
    registerTab: "Create account",
    emailLabel: "Email",
    passwordLabel: "Password",
    passwordHint: "At least 8 characters",
    nameLabel: "Name",
    farmNamePlaceholder: "e.g. My Farm",
    loginButton: "Sign in",
    registerButton: "Create account",
    workingLabel: "One moment…",
    invalidCredentials: "Wrong email or password",
    emailInUse: "This email is already registered — try signing in",
    passwordTooShort: "Password too short — at least 8 characters",
    invalidEmail: "Invalid email address",
    genericError: "Could not reach the server — check your connection and try again",
    accountHeading: "Account",
    yourNameHeading: "Your name",
    yourNameLabel: "Display name",
    yourNamePlaceholder: "e.g. Ahmed",
    nameSaveButton: "Save",
    nameSavedToast: "Name saved",
    farmLabel: "Farm",
    roleOwner: "Owner",
    roleWorker: "Supervisor",
    logoutButton: "Sign out",
    logoutConfirm:
      "You will be signed out and this device's local data cleared (it stays safe on the server). Continue?",
    logoutBlockedUnsynced:
      "This device has changes that haven't synced to the server yet. Connect to the internet and try again — signing out now would lose them.",
    switchFarmConfirm:
      "Switch this device to show the selected farm? Each farm keeps its own local copy on this device, so nothing is lost.",
    chooseFarmTitle: "Choose a farm",
    chooseFarmSubtitle: "Your account belongs to more than one farm — pick which one this device shows.",
    membersHeading: "Farm members",
    membersDescription:
      "Create a supervisor account with an email and password — they sign in with those directly, no self-registration needed.",
    addMemberNamePlaceholder: "Supervisor's name (optional)",
    addMemberEmailPlaceholder: "Supervisor's email",
    addMemberPasswordPlaceholder: "Supervisor password (min 8 chars)",
    addMemberButton: "Add supervisor",
    memberAddedToast: "Supervisor added",
    memberRemovedToast: "Supervisor removed",
    removeMemberConfirm: "Remove this supervisor from the farm?",
    userNotFound: "No account with this email",
    cannotRemoveLastOwner: "Cannot remove the farm's only owner",
    youBadge: "You",
    editPermissionsButton: "Allowed pages",
    permissionsHeading: "Allowed pages for",
    permissionsDescription: "Choose which pages this supervisor can see.",
    permissionsAllPages: "All pages",
    permissionsSaveButton: "Save",
    permissionsSavedToast: "Permissions saved",
    permissionsCancelButton: "Cancel",
    changePasswordHeading: "Change password",
    currentPasswordLabel: "Current password",
    newPasswordLabel: "New password",
    changePasswordButton: "Change password",
    passwordChangedToast: "Password changed",
    wrongCurrentPassword: "Current password is incorrect",
    farmDetailsHeading: "Farm details",
    farmNameLabel: "Farm name",
    farmLocationLabel: "Location",
    farmLocationPlaceholder: "City / region",
    farmSaveButton: "Save",
    farmSavedToast: "Farm details saved",
    farmNameRequired: "Please enter a farm name",
  },
  mobileSettings: {
    backupHeading: "Backup & Restore",
    backupDescription:
      "Backup: saves a copy of the database to a file. Restore: replaces all data — here and on the server for every device — with a saved file. Reset: permanently deletes all data from every device.",
    backupButton: "Backup",
    backingUpLabel: "Backing up…",
    backupSuccessToast: "Backup saved",
    restoreButton: "Restore",
    restoringLabel: "Restoring…",
    restoreConfirm:
      "This will replace all of this device's data AND the central online database (for every device) with the contents of the selected file. Continue?",
    restoreSuccessToast: "Restore complete",
    restoreInvalidFile: "The selected file is not a valid backup",
    restoreUploadFailed:
      "Restored on this device, but the central online database could not be updated — check your connection and restore from the same file again.",
    resetButton: "Reset",
    resettingLabel: "Resetting…",
    resetConfirm: (pendingCount: number) =>
      pendingCount > 0
        ? `A backup will be saved automatically first, then ALL farm data will be permanently deleted — from this device, the server, and every other device — including ${pendingCount} change${pendingCount === 1 ? "" : "s"} not yet synced. This can only be undone by restoring that backup. Are you sure?`
        : "A backup will be saved automatically first, then ALL farm data will be permanently deleted — from this device, the server, and every other device. This can only be undone by restoring that backup. Are you sure?",
    resetSuccessToast: "Database reset",
    resetOffline: "Reset needs an internet connection to wipe the central database too — nothing was deleted. Check your connection and try again.",
    resetOpsButton: "Clear operations",
    resettingOpsLabel: "Clearing…",
    resetOpsConfirm: (pendingCount: number) =>
      pendingCount > 0
        ? `A backup will be saved automatically first, then every recorded operation (matings, kindlings, weanings, weights, health, finance, stock, deaths, cullings…) will be permanently deleted — from this device, the server, and every other device — including ${pendingCount} change${pendingCount === 1 ? "" : "s"} not yet synced. Living rabbits, bucks, and breeds are kept (rabbits recorded as deceased or culled are deleted). This can only be undone by restoring that backup. Are you sure?`
        : "A backup will be saved automatically first, then every recorded operation (matings, kindlings, weanings, weights, health, finance, stock, deaths, cullings…) will be permanently deleted — from this device, the server, and every other device. Living rabbits, bucks, and breeds are kept (rabbits recorded as deceased or culled are deleted). This can only be undone by restoring that backup. Are you sure?",
    resetOpsSuccessToast: "All operations cleared",
  },
  kindling: {
    title: "Kindling",
    description: (count: number, days: number) =>
      `${count} doe${count === 1 ? "" : "s"} due to kindle (${days} days after mating).`,
    colIndex: "#",
    colDoeTag: "Doe #",
    colBreed: "Breed",
    colBuckTag: "Buck #",
    colMatingDate: "Mating date",
    colExpectedKindling: "Expected kindling",
    colDoeState: "Doe state",
    colKindle: "Kindle",
    kindleButton: "Kindle",
    emptyTitle: "No does are due to kindle right now",
    emptyDescription:
      "Pregnant does will show up here once they pass the gestation length set in Settings.",
    logHeading: "Kindling log",
    logEmptyTitle: "No kindlings recorded yet",
    logEmptyDescription:
      "Any doe whose kindling gets recorded will show up here with its mating and kindling dates.",
    colKindlingDate: "Kindling date",
    colBornAlive: "Born alive",
    colBornDead: "Stillborn",
    // Kits here = born alive only, stillborn excluded — avg litter likewise.
    totalKitsBadge: "Total live kits",
    avgLitterBadge: "Avg live litter",
  },
  validation: {
    invalidValue: "Invalid value",
    rebreedMaxDays: (max: number) =>
      `The rebreed interval can't be more than ${max} days — set to ${max}.`,
    weaningMaxDays: (max: number) =>
      `The weaning wait can't be more than ${max} days (30 + rebreed interval − 2) — set to ${max}.`,
    invalidCurrency: "Invalid currency code — use an ISO code like EGP or USD",
    required: "Required",
    invalidDate: "Invalid date",
    weightPositive: "Enter a weight greater than 0",
    sexRequired: "Sex is required",
    parentsSame: "Sire and dam cannot be the same rabbit",
    buckRequired: "Buck is required",
    doeRequired: "Doe is required",
    buckDoeSame: "Buck and doe must be different rabbits",
    weanedExceedsBornAlive: "Weaned can't exceed the number born alive",
    fosterCountMin: "Count must be at least 1",
    fosterSameDoe: "The from-doe and to-doe can't be the same doe",
    amountPositive: "Amount must be greater than 0",
    breedNameRequired: "Breed name is required",
    breedNameTooLong: "Breed name is too long",
    rabbitTagLabel: "Rabbit #",
    motherTagLabel: "Doe #",
    buckTagLabel: "Buck #",
    cageTagLabel: "Cage #",
    fromDoeTagLabel: "From doe #",
    toDoeTagLabel: "To doe #",
    tagRequired: (label: string) => `${label} is required`,
    tagTooLong: (label: string) => `${label} is too long`,
    tagInvalidChars: (label: string) => `${label} may only contain letters and numbers`,
  },
  doeStateMenu: {
    stateSetToast: (stateLabel: string) => `Doe state set: ${stateLabel}`,
    mateButton: "Mate",
    buckTagPlaceholder: "Buck #",
    buckNotFoundToast: (buckTag: string) => `No buck with #${buckTag} — mating was not recorded`,
    matedTodayToast: "Mating recorded for today",
    matedToast: "Mating recorded",
    matingDateUsedToast: "This doe already has a mating recorded on that date",
    matingDateUsedShort: "Date already used",
    tagNotFound: (value: string) => `#${value} not found`,
    kindledToast: "Kindling recorded",
    bornAlivePlaceholder: "Alive",
    bornDeadPlaceholder: "Dead",
    nestBoxInstalledToast: "Nest box install recorded",
    installButton: "Install",
    weanedTodayToast: "Weaning recorded for today",
    weanNeedsCounts: "Enter the weaned count and weight first",
    matingDateUpdatedToast: "Mating date updated",
    invalidValueFallback: "Invalid value",
    clearConfirm:
      'This will permanently clear all of this row\'s data (mating, kindling, weaning) and reset the doe to "Empty". Are you sure?',
    clearedToast: "Doe data cleared and reset to empty",
    matingFailedToast: "Mating canceled and its date cleared",
    cullConfirm:
      "Culling permanently removes this rabbit from the herd, and this step cannot be undone. Are you sure?",
    palpationConfirmedToast: "Pregnancy confirmed as ongoing",
    resorptionConfirm:
      'Fetuses disappearing means resorption occurred and the pregnancy has ended. This will be logged in the resorption log and the doe reset to "Empty". Are you sure?',
    resorptionToast: "Resorption logged and doe reset to empty",
  },
  dashboard: {
    heroTitle: "Dashboard",
    heroDescription: "Everything that needs your attention today on your farm.",
    activeRabbits: "Active rabbits",
    activeDoes: "Active Does",
    activeBucks: "Active Bucks",
    stockCount: "Strains (stock)",
    upcomingKindlings: "Upcoming kindlings",
    overdueTasks: "Overdue tasks",
    weaningSurvivalRate: "Weaning survival rate",
    readyForMating: "Does ready for mating",
    readyForPregnancyTest: "Ready for pregnancy test",
    expectedKindlings: "Expected kindlings",
    nestBoxesDue: "Nest boxes due",
    kindlingsHeading: "Kindlings",
    viewAll: "View all",
    noPendingBreedings: "No breedings pending.",
    overdueDays: (days: number) => `${days} day${days === 1 ? "" : "s"} overdue`,
    dueOn: "Due",
    healthTasksHeading: "Health tasks",
    noUpcomingHealth: "Nothing due soon.",
    overdueHealthDays: (days: number) => `${days} day${days === 1 ? "" : "s"} overdue`,
    herdByStatus: (total: number) => `Herd by status (${total})`,
    recentLittersHeading: "Recent litters",
    noWeanedLitters: "No weaned litters yet.",
    stockFallback: "Juvenile",
  },
  daily: {
    title: "Daily",
    description: "Everything logged today: matings, pregnancy tests, nest box installs, kindlings, weanings, and deaths.",
    dateLabel: "Date",
    prevDay: "Previous day",
    nextDay: "Next day",
    today: "Today",
    applyButton: "View",
    colIndex: "#",
    colMotherTag: "Doe #",
    colBreed: "Breed",
    colBuckTag: "Buck #",
    colResult: "Result",
    colAlive: "Alive",
    colDead: "Dead",
    colWeaned: "Weaned",
    colWeight: "Weight (g)",
    colSex: "Sex",
    colTag: "Tag",
    colStatus: "Status",
    resultPositive: "Positive",
    resultNegative: "Negative",
    matingsHeading: (count: number) => `Matings (${count})`,
    matingsEmpty: "No matings recorded on this day",
    pregnancyHeading: (count: number) => `Pregnancy tests (${count})`,
    pregnancyEmpty: "No pregnancy tests recorded on this day",
    nestBoxHeading: (count: number) => `Nest box installs (${count})`,
    nestBoxEmpty: "No nest box installs recorded on this day",
    kindlingHeading: (count: number) => `Kindlings (${count})`,
    kindlingEmpty: "No kindlings recorded on this day",
    weaningHeading: (count: number) => `Weanings (${count})`,
    weaningEmpty: "No weanings recorded on this day",
    mortalityHeading: (count: number) => `Deaths & culls (${count})`,
    mortalityEmpty: "No deaths or culls recorded on this day",
    kitDeathsHeading: (count: number) => `Nursing kit deaths (${count})`,
    kitDeathsEmpty: "No nursing kit deaths recorded on this day",
    weanedKitDeathsHeading: (count: number) => `Weaned kit deaths (${count})`,
    weanedKitDeathsEmpty: "No weaned kit deaths recorded on this day",
    colCount: "Count",
  },
  stock: {
    title: "Juveniles",
    description:
      "Quick intake for young rabbits before they're promoted to the herd with a number and weight.",
    noAvailableStock:
      "No available stock — record a weaning or an adjustment in Weaning & Sales before registering new breeding stock.",
    sexDoe: "Doe",
    sexBuck: "Buck",
    registeredToast: "Juvenile registered",
    sexLabel: "Sex",
    dateLabel: "Date",
    breedLabel: "Breed",
    weightLabel: "Weight (kg)",
    cageLabel: "Cage #",
    submitButton: "Register juvenile",
    colDate: "Date",
    colSex: "Sex",
    colBreed: "Breed",
    colCage: "Cage #",
    colWeight: "Weight",
    cagePlaceholder: "Cage #",
    cageSavedToast: "Cage # saved",
    invalidCageFallback: "Invalid cage number",
    weightPlaceholder: "kg",
    weightSavedToast: "Weight saved",
    invalidWeightFallback: "Invalid weight",
    promoteToDoeLine: "Move to does line",
    promoteToBuckLine: "Move to bucks line",
    movedToast: "Moved",
    moveFailedFallback: "Could not move",
    // A warning, not a bare "are you sure?": the delete puts the animal back
    // into the weaned-kit balance, so it has to be physically moved to the
    // weaning cages or the balance and the pens stop matching.
    deleteConfirm:
      "⚠️ Warning: deleting this juvenile returns it to the weaned-kit balance as \"Returned from juveniles\".\n\nYou must physically move the rabbit to the weaning cages, otherwise the balance will not match what is in the pens.\n\nContinue?",
    deletedToast: "Juvenile deleted and returned to the weaned-kit balance",
    deleteFailedFallback: "Could not delete juvenile",
  },
  bucks: {
    title: "Bucks",
    description: (count: number) => `${count} buck${count === 1 ? "" : "s"} in the herd`,
    pendingHeading: (count: number) => `Juveniles awaiting a buck number (${count})`,
    emptyTitle: "No bucks in the herd yet",
    emptyDescription:
      "Register a male juvenile from the Juveniles page, and once it has a cage number it'll show up in the section above so you can assign its buck number and add it here.",
    colIndex: "#",
    colTag: "Tag",
    colBreed: "Breed",
    colAddedDate: "Date added to herd",
    colWeight: "Weight",
    colStatus: "Status",
    colEdit: "Edit",
    editCardAria: (tag: string) => `Edit buck #${tag} card`,
    addedToast: (tag: string) => `Buck #${tag} added`,
    addFormTitle: "Add Buck (Purchased External — No Weaning Deduction)",
    tagLabel: "Buck #",
    tagPlaceholder: "e.g. 42 or 5A",
    breedLabel: "Breed",
    weightLabel: "Weight (kg)",
    weightPlaceholder: "Optional",
    submitButton: "Add Buck (No Deduction)",
    pendingEmptyTitle: "No bucks pending",
    pendingEmptyDescription:
      "A male juvenile assigned a cage on the Juveniles page will show up here until you assign its buck number.",
    colCage: "Cage #",
    colBuckTag: "Buck #",
    finalizedToast: "Added to bucks",
    weightPlaceholderShort: "kg",
    tagPlaceholderShort: "Buck #",
    finalizeButton: "Add to bucks",
  },
  mothers: {
    title: "Does",
    description: (count: number) => `${count} doe${count === 1 ? "" : "s"} in the herd`,
    pendingHeading: (count: number) => `Juveniles awaiting a doe number (${count})`,
    emptyTitle: "No does in the herd yet",
    emptyDescription:
      "Register a female juvenile from the Juveniles page, and once it has a cage number it'll show up in the section above so you can assign its doe number and add it here.",
    colIndex: "#",
    colTag: "Tag",
    colBreed: "Breed",
    colAddedDate: "Date added to herd",
    colWeight: "Weight",
    colStatus: "Status",
    colDoeState: "Reproductive state",
    colEdit: "Edit",
    editCardAria: (tag: string) => `Edit doe #${tag} card`,
    addedToast: (tag: string) => `Doe #${tag} added`,
    addFormTitle: "Add Doe (Purchased External — No Weaning Deduction)",
    tagLabel: "Doe #",
    tagPlaceholder: "e.g. 42 or 5A",
    breedLabel: "Breed",
    weightLabel: "Weight (kg)",
    weightPlaceholder: "Optional",
    submitButton: "Add Doe (No Deduction)",
    pendingEmptyTitle: "No does pending",
    pendingEmptyDescription:
      "A female juvenile assigned a cage on the Juveniles page will show up here until you assign its doe number.",
    colCage: "Cage #",
    colMotherTag: "Doe #",
    finalizedToast: "Added to does",
    weightPlaceholderShort: "kg",
    tagPlaceholderShort: "Doe #",
    finalizeButton: "Add to does",
  },
  does: {
    title: "Farm Operations",
    description: "Track mating and pregnancy testing for every doe.",
    emptyTitle: "No does in the herd yet",
    emptyDescription:
      "Add any juvenile's number from the add-rabbit page (Add to herd button) so it shows up here.",
    colIndex: "#",
    colMotherTag: "Doe #",
    colBreed: "Breed",
    colDoeState: "Doe state",
    colMate: "Mate",
    colMatingDate: "Mating date",
    colTestDate: "Test date",
    colTestResult: "Test result",
    colPalpation: "Confirm palpation",
    colKindlingDate: "Kindling date",
    colKindle: "Kindle",
    colBornCount: "Born",
    colBornAlive: "Alive",
    colBornDead: "Dead",
    colWean: "Wean",
    colWeanedCount: "Weaned",
    colWeaningDate: "Weaning date",
    colWeaningWeight: "Weaning wt (g)",
    colClear: "Clear",
    pregnantButton: "Pregnant",
    negativeButton: "Negative",
    kindleButton: "Kindle",
    weanButton: "Wean",
    clearButton: "Clear",
    confirmPregnantButton: "Confirm pregnant",
    resorptionButton: "Fetuses disappeared",
  },
  rounds: {
    title: "Farm Rounds",
    description: "Walk through every doe and record kindling, deaths, or illness from the same screen — data flows automatically to the other pages.",
    emptyTitle: "No does in the herd yet",
    emptyDescription: "Add any doe's number from the add-rabbit page so it shows up here.",
    kindlingLabel: "Kindling",
    notKindledYetNote: "Hasn't kindled yet",
    pregnantDaysNote: (days: number) => `Pregnant, day ${days}`,
    palpationLabel: "Confirm palpation",
    palpationNotDueNote: "Not due for palpation confirmation yet",
    nursingDeathLabel: "Nursing kit death",
    noNursingLitterNote: "No nursing litter right now",
    doeDeathButton: "Record doe death",
    healthLabel: "Health",
    healthAddButton: "Log illness/treatment",
    healthCancelButton: "Cancel",
    healthSaveButton: "Save",
    healthSavedToast: "Health record logged",
    healthDescriptionPlaceholder: "e.g. mange symptoms, not eating...",
    healthDescriptionRequired: "Please enter a description",
  },
  bucksRounds: {
    title: "Buck Rounds",
    description: "Walk through every buck and record a death or illness from the same screen — data flows automatically to the other pages.",
    emptyTitle: "No bucks in the herd yet",
    emptyDescription: "Add any buck's number from the add-rabbit page so it shows up here.",
    healthLabel: "Health",
    healthAddButton: "Log illness/treatment",
    healthCancelButton: "Cancel",
    healthSaveButton: "Save",
    healthSavedToast: "Health record logged",
    healthDescriptionPlaceholder: "e.g. mange symptoms, not eating...",
    healthDescriptionRequired: "Please enter a description",
    deathButton: "Record buck death",
  },
  mating: {
    title: "Mating",
    description: (count: number) => `${count} doe${count === 1 ? "" : "s"} ready to mate now.`,
    emptyTitle: "No does ready to mate right now",
    emptyDescription:
      "Does that are bred, pregnant, or nursing will show up here again once they start a new cycle.",
    colIndex: "#",
    colMotherTag: "Doe #",
    colBreed: "Breed",
    colDoeState: "Doe state",
    colDoeStateAtMating: "Doe state at mating",
    colMate: "Mate",
    logHeading: "Mating log",
    logEmptyTitle: "No matings recorded yet",
    logEmptyDescription: "Any doe that gets mated will show up here with her mating date.",
    colBuckTag: "Buck #",
    colMatingDate: "Mating date",
  },
  nestBox: {
    title: "Nest Boxes",
    description: (count: number, days: number) =>
      `${count} doe${count === 1 ? "" : "s"} due for a nest box (${days} days after mating).`,
    emptyTitle: "No does need a nest box right now",
    emptyDescription:
      "Pregnant does will show up here once they pass the nest box wait time set in Settings.",
    colIndex: "#",
    colMotherTag: "Doe #",
    colBreed: "Breed",
    colBuckTag: "Buck #",
    colMatingDate: "Mating date",
    colExpectedInstallDate: "Expected install date",
    colDoeState: "Doe state",
    colInstall: "Install nest box",
    logHeading: "Nest box log",
    logEmptyTitle: "No nest box installs recorded yet",
    logEmptyDescription:
      "Any doe whose nest box gets installed will show up here with her mating and install dates.",
    colInstallDate: "Install date",
  },
  pregnancyTest: {
    title: "Pregnancy Tests",
    resultPositive: "Positive",
    resultNegative: "Negative",
    description: (count: number, days: number) =>
      `${count} doe${count === 1 ? "" : "s"} due for a pregnancy test (${days} days after mating).`,
    emptyTitle: "No does are due for a pregnancy test right now",
    emptyDescription:
      "Mated does will show up here once they pass the pregnancy test wait time set in Settings.",
    colIndex: "#",
    colMotherTag: "Doe #",
    colBreed: "Breed",
    colBuckTag: "Buck #",
    colMatingDate: "Mating date",
    colTestDate: "Test date",
    colDoeState: "Doe state",
    colTestResult: "Test result",
    pregnantButton: "Pregnant",
    negativeButton: "Negative",
    // Fertility rate = positives ÷ all tests recorded in the same range.
    positiveCountBadge: "Pregnant",
    fertilityRateBadge: "Fertility rate",
    logHeading: "Test log",
    logEmptyTitle: "No tests recorded yet",
    logEmptyDescription:
      "Any doe that gets tested (positive or negative) will show up here with her mating date, test date, and result.",
  },
  resorptionLog: {
    title: "Resorption Log",
    colIndex: "#",
    colMotherTag: "Doe #",
    colBreed: "Breed",
    colBuckTag: "Buck #",
    colMatingDate: "Mating date",
    colResorptionDate: "Resorption date",
    logHeading: "Resorption log",
    logEmptyTitle: "No resorptions recorded yet",
    logEmptyDescription:
      "Any doe that resorbs a pregnancy will show up here with her mating date and resorption date.",
  },
  weaning: {
    title: "Weaning",
    description: (count: number, days: number) =>
      `${count} doe${count === 1 ? "" : "s"} due to wean (${days} days after kindling).`,
    emptyTitle: "No does are due to wean right now",
    emptyDescription:
      "Nursing does will show up here once they pass the weaning wait time set in Settings.",
    colIndex: "#",
    colMotherTag: "Doe #",
    colBreed: "Breed",
    colBuckTag: "Buck #",
    colKindlingDate: "Kindling date",
    colExpectedWeaningDate: "Expected weaning date",
    colDoeState: "Doe state",
    colAlive: "Alive",
    colDead: "Dead",
    colWean: "Wean",
    weanButton: "Wean",
    // Rows with no weaned count yet (shown as "—") stay out of the total.
    // A weaning is not a kit — this badge counts rows, next to a total of kits.
    countUnit: "weanings",
    totalWeanedBadge: "Total weaned",
    avgWeanedBadge: "Avg weaned per doe",
    logHeading: "Weaning log",
    logEmptyTitle: "No weanings recorded yet",
    logEmptyDescription:
      "Any doe whose weaning gets recorded will show up here so you can enter the weaned count.",
    colWeaningDate: "Weaning date",
    colWeanedCount: "Weaned",
    colWeaningWeight: "Weight (g)",
    colSurvivalRate: "Survival rate",
    groupNursingCount: "Under care",
  },
  weaningSales: {
    title: "Weaning & Sales",
    description: "A daily log of weaned, sold, and dead kit counts, and the current available stock.",
    totalWeanedLabel: "Total weaned",
    availableStockLabel: "Currently available",
    totalSoldLabel: "Total sold",
    totalDiedLabel: "Total died",
    totalRetainedLabel: "Kept for breeding",
    soldWeightLabel: "Weight",
    avgSoldWeightLabel: "Avg weight",
    soldRevenueLabel: "Total",
    saleFormHeading: "Record a sale",
    dateLabel: "Date",
    countLabel: "Count",
    totalWeightLabel: "Total weight (kg)",
    pricePerKgLabel: (currency: string) => `Price per kg (${currency})`,
    notesLabel: "Notes",
    saleSubmitButton: "Record sale",
    saleAddedToast: "Sale recorded",
    deletedToast: "Entry deleted",
    colDate: "Date",
    colType: "Type",
    colCount: "Count",
    colWeight: "Weight",
    colPricePerKg: "Price/kg",
    colAmount: "Amount",
    colNotes: "Notes",
    typeWean: "Weaned",
    typeSale: "Sold",
    typeDeath: "Died",
    typeRetained: "Kept for breeding",
    typeAdjustment: "Adjustment",
    // A deleted juvenile going back to the weaning cages — adds to the
    // balance instead of withdrawing from it.
    typeReturned: "Returned from juveniles",
    emptyTitle: "No movements recorded yet",
    emptyDescription:
      "Weaned counts show up here automatically. Record a sale with the form above, record deaths from the mortality census page, and kits kept for breeding are deducted automatically when a juvenile is added to the herd.",
  },
  fostering: {
    title: "Fostering",
    pageTitle: "Fostering",
    description: "Move nursing kits from one doe to another to balance litter sizes.",
    logTitle: "Fostering log",
    // A row is a transfer, not a kit — the badge counts transfers.
    countUnit: "transfers",
    emptyTitle: "No fostering events recorded",
    emptyDescription: "Record a kit transfer between does using the form above.",
    kitsUnit: (count: number) => `${count} kit${count === 1 ? "" : "s"}`,
    successToast: "Fostering transfer recorded",
    fromLabel: "From (doe # transferred from)",
    toLabel: "To (doe # transferred to)",
    countLabel: "Count",
    submitButton: "Save",
    highTitle: "Large litters — move kits out",
    highDescription: "Kindled in the last {days} day(s) and nursing {n} kits or more.",
    highEmpty: "No does with a large litter in this window",
    lowTitle: "Small litters — move kits in",
    lowDescription: "Kindled in the last {days} day(s) and nursing {n} kits or fewer.",
    lowEmpty: "No does with a small litter in this window",
    colIndex: "#",
    colDoe: "Doe #",
    colCage: "Cage",
    colKindlingDate: "Kindling date",
    colKits: "Nursing kits",
    // Export-only columns: the log itself reads as «from → to» on one line.
    colDate: "Date",
    colFromDoe: "From doe",
    colToDoe: "To doe",
  },
  mortality: {
    title: "Mortality",
    description: "Record daily deaths: a nursing kit, a doe, a buck, or a juvenile.",
    nursingSectionTitle: "Nursing kit deaths",
    nursingEmptyTitle: "No nursing does right now",
    nursingEmptyDescription: "Does with live nursing kits will show up here so you can record a kit death.",
    nursingFormHint: "Enter a doe's number and press “Show” to add her to the table and record her kit deaths.",
    nursingMotherNotFound: "No nursing doe with this number",
    showRowButton: "Show",
    removeRowLabel: "Remove row",
    colIndex: "#",
    colMotherTag: "Doe #",
    colBreed: "Breed",
    colAlive: "Alive",
    colDead: "Dead",
    colRecordDeath: "Record death",
    mothersSectionTitle: "Doe deaths",
    mothersEmptyTitle: "No does registered",
    mothersFormHint: "Enter the deceased doe's number, then press “Record deceased”.",
    motherTagPlaceholder: "Enter doe #",
    motherNotFound: "No doe with this number in the herd",
    colRecordDeceased: "Record deceased",
    motherDeathConfirm: (tag: string) =>
      `Record death of doe #${tag}? She'll be moved to the deceased does table.`,
    bucksSectionTitle: "Buck deaths",
    bucksEmptyTitle: "No bucks registered",
    bucksFormHint: "Enter the deceased buck's number, then press “Record deceased”.",
    buckTagPlaceholder: "Enter buck #",
    buckNotFound: "No buck with this number in the herd",
    colBuckTag: "Buck #",
    buckDeathConfirm: (tag: string) =>
      `Record death of buck #${tag}? He'll be moved to the deceased bucks table.`,
    strainsSectionTitle: "Juvenile deaths",
    strainsEmptyTitle: "No juveniles registered",
    strainsFormHint: "Enter a cage number to list the juveniles in it.",
    cagePlaceholder: "Enter cage #",
    cageNotFound: "No juveniles in this cage",
    colSex: "Sex",
    colStrainBreed: "Breed",
    colCage: "Cage #",
    strainDeathConfirm: "Record death of this juvenile? It'll be moved to the deceased juveniles table.",
    deceasedMothersHeading: (count: number) => `Deceased does (${count})`,
    deceasedMothersEmptyTitle: "No deceased does",
    deceasedBucksHeading: (count: number) => `Deceased bucks (${count})`,
    deceasedBucksEmptyTitle: "No deceased bucks",
    deceasedStrainsHeading: (count: number) => `Deceased juveniles (${count})`,
    deceasedStrainsEmptyTitle: "No deceased juveniles",
    colRegisteredDate: "Date recorded",
    nursingKitDeathConfirm: (count: number) =>
      `Record ${count} death${count === 1 ? "" : "s"} among this doe's nursing kits?`,
    recordDeathFailedFallback: "Could not record death",
    kitDeathToast: (count: number) => `Recorded ${count} kit death${count === 1 ? "" : "s"}`,
    nursingKitCountInputLabel: "Count",
    weaningStockSectionTitle: "Weaned stock deaths",
    availableWeanedStockLabel: "Available after weaning",
    weaningStockCountInputLabel: "Count",
    weaningStockDeathConfirm: (count: number) =>
      `Record ${count} death${count === 1 ? "" : "s"} from the available weaned stock?`,
    weaningStockDeathToast: (count: number) =>
      `Recorded ${count} death${count === 1 ? "" : "s"} from weaned stock`,
    weaningStockDeathCountExceedsAvailable: "Requested count exceeds the available stock",
    recordWeaningDeathButton: "Record death",
    noWeanedStockAvailable: "No weaned stock available to record a death",
    recordNursingDeathButton: "Record death",
    deceasedToast: "Death recorded",
    recordDeceasedButton: "Record deceased",
    logTabKitDeaths: "Nursing kits",
    logTabWeanedKitDeaths: "Weaned kits",
    logTabDeceasedMothers: "Deceased does",
    logTabDeceasedBucks: "Deceased bucks",
    logTabDeceasedStrains: "Deceased juveniles",
    kitDeathsHeading: (count: number) => `Nursing kit deaths (${count})`,
    kitDeathsEmptyTitle: "No nursing kit deaths recorded",
    weanedKitDeathsHeading: (count: number) => `Weaned kit deaths (${count})`,
    weanedKitDeathsEmptyTitle: "No weaned kit deaths recorded",
    colDeathDate: "Date of death",
    colCount: "Count",
    colKindlingDate: "Kindling date",
    kitDeathsTotalLabel: "Total kits lost",
    // Count lives in <LogCountBadge> next to the heading, not in the string.
    culledHeading: "Culling record",
    culledEmptyTitle: "No culled rabbits",
    colTag: "#",
  },
  health: {
    title: "Health",
    description: "Recurring appointments and recent health events across the herd.",
    overdueHeading: (count: number) => `Overdue (${count})`,
    overdueEmpty: "Nothing overdue. 🎉",
    overdueDays: (days: number) => `${days} day${days === 1 ? "" : "s"} overdue`,
    dueOn: "Due",
    upcomingHeading: (count: number) => `Upcoming (30 days) (${count})`,
    upcomingEmpty: "Nothing due in the next 30 days.",
    inDays: (days: number) => `in ${days} day${days === 1 ? "" : "s"}`,
    recentEventsHeading: "Recent events",
    emptyTitle: "No health records yet",
  },
  reports: {
    title: "Reports",
    description: "All statistical reports and doe & farm fertility records in one place.",
    tabFollowUp: "Follow-up Reports",
    tabDoesFertility: "Does Fertility Report",
    tabBucksFertility: "Bucks Fertility Report",
    tabHerdProductivity: "Herd Productivity",
    tabIdleDoes: "Idle Does",
    tabWeakDoes: "Underperforming Does",
    tabTopDoes: "Best Does",
    fromLabel: "From",
    toLabel: "To",
    applyButton: "View report",
    rangeWeekButton: "Week",
    rangeMonthButton: "Month",
    rangeQuarterButton: "3 months",
    rangeYearButton: "Year",
    rangeAllButton: "Since day one",
    notTrackedNote: "Some fields aren't tracked by the system yet and show as (—).",
    allTimeBadge: "Current balance",
    allTimeNote: "These cards show the farm's current balance and are not affected by the date range below.",
    sectionHerd: "Herd",
    doesLabel: "Does",
    bucksLabel: "Bucks",
    sectionWeanedBalance: "Weaned stock now",
    weanedBalanceLabel: "Available to sell",
    weanedBalanceNote: "What is still standing in the weaning cages: everything weaned, less what was sold, died, or kept back as juveniles.",
    sectionCull: "Does to cull",
    cullLabel: "Does",
    cullNote: (threshold: number, minMatings: number) =>
      `Does whose fertility is under ${threshold}% — kindlings ÷ matings over their whole history. A doe with fewer than ${minMatings} matings is not counted, so a new doe never reads as a failure.`,
    sectionCullBucks: "Bucks to cull",
    cullBucksLabel: "Bucks",
    cullBucksNote: (threshold: number, minMatings: number) =>
      `Bucks whose fertility is under ${threshold}% — confirmed pregnancies ÷ matings over their whole history. A buck's job is settling the doe; whether she carries to term is hers. A buck with fewer than ${minMatings} matings is not counted.`,
    sectionStock: "Juveniles",
    stockMalesLabel: "Juvenile males",
    stockFemalesLabel: "Juvenile females",
    stockAgeUnder1mLabel: "Under 1 month",
    stockAge1to2mLabel: "1 to 2 months",
    stockAge2to3mLabel: "2 to 3 months",
    stockAgeOver3mLabel: "Over 3 months",
    sectionDeaths: "Deaths",
    totalDeathsLabel: "Total deaths (newborn + weaned)",
    newbornDeathsLabel: "Newborn",
    weanedStockDeathsLabel: "Weaned stock",
    stockDeathsLabel: "Juvenile deaths",
    doeDeathsLabel: "Doe deaths",
    buckDeathsLabel: "Buck deaths",
    culledExcludedDeathsLabel: "Excluded/culled deaths",
    cullsLabel: "Culls",
    sectionWeaning: "Weaning & sales",
    totalWeanedLabel: "Total weaned",
    soldLabel: "Sold",
    retainedLabel: "Retained during period (juveniles)",
    remainingStockLabel: (date: string) => `Weaned stock available on ${date}`,
    remainingStockNowLabel: "Weaned stock available now",
    sectionHealth: "Health cases",
    mangeStockLabel: "Mange (juveniles)",
    mangeDoesLabel: "Mange (does)",
    mangeBucksLabel: "Mange (bucks)",
    uterineInfectionLabel: "Uterine infection",
    mastitisLabel: "Mastitis",
    sectionBreeding: "Breeding & Kindling",
    matingsLabel: "Matings",
    pregnancyPositiveLabel: "Pregnant",
    kindlingsLabel: "Kindlings",
    sectionAverages: "Averages",
    avgLittersPerDoeYearLabel: "Litters per doe per year",
    avgBornAliveLabel: "Avg. live litter size",
    avgWeanedLabel: "Avg. kits weaned per litter",
    avgSoldPerWeaningLabel: "Avg. kits sold per litter",
    avgSalesPerDoeLabel: "Sales per doe per month on the farm",
    // The unit lives in the label, as it does on the herd tab's twin tile.
    avgWeightPerDoeLabel: "Avg. kg sold per doe per month",
    avgRevenuePerDoeLabel: "Avg. revenue per doe per month",
    avgMonthlySalesLabel: "Average monthly sales",
    avgMonthsBasis: (count: number) => `÷ ${count} months since the first selling month`,
    avgKindlingBasis: (count: number) => `÷ ${count} kindlings since the farm started`,
    avgLittersPerDoeYearBasis: (litters: number) => `${litters} litters`,
    avgWeaningBasis: (count: number) => `÷ ${count} weanings since the farm started`,
    avgFunnelBasis: "Birth to sale",
    avgSellingBasis: "Monthly selling",
    avgLaggedMonthsBasis: (count: number) => `Mean of ${count} months`,
    avgSalesPerDoeBasis: (count: number) => `Mean of ${count} months`,
    avgWeightPerDoeBasis: (count: number) => `Mean of ${count} months`,
    avgRevenuePerDoeBasis: (pricePerKg: string) => `${pricePerKg} per kg, from settings`,
    avgRevenuePerDoeNoPriceBasis: "Set the price per kg in settings to see revenue",
    avgUnknownWeightMonthsNote: (count: number) =>
      `${count} month(s) sold kits with no weight on the record and are excluded from the weight average rather than counted as zero.`,
    avgAllTimeBadge: "All time",

    // ——— Weaned stock curve ———
    sectionSalesChart: "Monthly sales since the farm started",
    sectionSalesStockChart: "Monthly sales and weaned stock since the farm started",
    salesChartSeriesLabel: "Sold",
    salesChartDoesLabel: "Active does",
    salesChartRatioLabel: "Sold per doe per month",
    salesChartEmpty: "No sales in a completed month yet.",

    sectionStockChart: "Weaned stock since the farm started",
    stockChartSeriesLabel: "Weaned stock",
    stockChartEmpty: "No weanings or stock movements recorded yet.",
    stockChartNote:
      "The running balance available to sell at each point in time: it rises with every weaning and falls with sales, deaths, and stock retained for breeding. Unaffected by the date filter below.",
    stockChartBucketDay: "Daily",
    stockChartBucketWeek: "Weekly",
    stockChartBucketMonth: "Monthly",

    // ——— Herd productivity ———
    herdNoDoesNote: "No does on file, so no rate can be computed.",

    herdSectionCycles: "Reproductive cycles",
    herdCyclesActualLabel: "Actual cycles per doe per year",
    herdCyclesTargetLabel: "Target from the rebreed system",
    herdCycleAchievementLabel: "Target achieved",
    herdCycleNote: (target: number, cycleDays: number) =>
      `The target (${target} cycles a year) comes from the rebreed interval set in settings — roughly one full cycle every ${cycleDays} days.`,
    herdCyclesActualBasis: (doeYears: string, excludesRunningMonth: boolean) =>
      `litters in range ÷ ${doeYears} doe-years — each doe counted for the time she actually stood, not by today's head count${
        excludesRunningMonth ? "; the running month is left out as incomplete" : ""
      }`,


    herdSectionMonthly: "Monthly return per doe",
    herdWeanedPerMonthLabel: "Weaned per doe per month",
    herdSoldPerMonthLabel: "Head sold per doe per month",
    herdKgSoldPerMonthLabel: "Kg sold per doe per month",
    herdRevenuePerMonthLabel: "Income per doe per month",
    herdCostPerMonthLabel: "Allocated cost per doe per month",
    herdNetPerMonthLabel: "Net per doe per month",
    herdNetPositiveNote: "Net is positive: on average a doe covers her keep and more.",
    herdNetNegativeNote:
      "Net is negative: the average doe brought in less than her share of the costs over this period.",

    herdSectionTotals: "Period totals",
    herdTotalIncomeLabel: "Total income",
    herdTotalExpenseLabel: "Total expenses",
    herdTotalNetLabel: "Net",
    herdSoldCountLabel: "Rabbits sold",
    herdSoldPerDoePerYearLabel: "Rabbits sold per doe per year",
    herdTotalsProfitNote: (net: string) => `The farm made ${net} over the selected period.`,
    herdTotalsLossNote: (net: string) => `The farm lost ${net} over the selected period.`,

    herdSectionBreakEven: "Break-even price",
    herdKgSoldLabel: "Total meat sold (kg)",
    herdRealizedPriceLabel: "Realized price per kg",
    herdBreakEvenPriceLabel: "Break-even price per kg",
    herdMarginPerKgLabel: "Margin per kg",
    herdFeedKgLabel: "Feed consumed (kg)",
    herdFeedConversionLabel: "Actual conversion (kg feed consumed / kg meat sold)",
    herdBreakEvenPositiveNote: (margin: string) => `Each kilo clears ${margin} above break-even.`,
    herdBreakEvenNegativeNote: (margin: string) =>
      `Each kilo falls ${margin} short of break-even — selling at today's price widens the gap.`,
    herdBreakEvenNoSales: "No meat was sold in this period, so no break-even price can be computed.",

    herdSectionIdle: "Idle does",
    herdIdleEmpty: "No idle does — every doe in the herd is within her cycle.",
    herdIdleCountLabel: "Idle does",
    herdIdleShareLabel: "Share of the herd",
    herdColIndex: "#",
    herdColTag: "Doe",
    herdColBreed: "Breed",
    herdColLastKindling: "Last kindling",
    herdColIdleDays: "Days without kindling",
    herdNeverKindled: "Never kindled",

    weakSectionTitle: "Underperforming does",
    weakEmpty: "No underperforming does — every doe in the barn is within the herd's own range.",
    weakCountLabel: "Underperforming does",
    weakShareLabel: "Share of the herd",
    weakHerdLitterLabel: "Farm average litter size",
    weakColScore: "Score",
    weakColMatings: "Matings",
    weakColKindlings: "Kindlings",
    weakColFertility: "Fertility",
    weakColLitterSize: "Avg litter size",
    weakColReasons: "Why she is listed",
    weakReasonFertility: "Low fertility",
    weakReasonLitterSize: "Small litters",
    // The whole method on one line, because this list is a spending decision:
    // an owner about to sell an animal is owed the rule that named her.
    weakNote: (fertilityThreshold: number, relativePct: number, minMatings: number) =>
      `Does that failed one or both of two tests: fertility under ${fertilityThreshold}%, or average litter size under ${relativePct}% of your farm's average. Litter size is measured against your own farm because what counts as a big litter differs by breed. A test is not run on a doe with fewer than ${minMatings} matings (or ${minMatings} kindlings), so a new doe never reads as a failure.`,
    // Lifetime, not the report's range — stated because every other tab on this
    // page answers a question about a window, and this one does not.
    weakLifetimeNote:
      "Computed over the doe's whole history, not a chosen period: culling is permanent, and a doe who had two bad months after a good year is not the same animal as one who has been weak since day one.",
    // The score is a single number about a live animal, so the rule behind it
    // is printed wherever it is: a grade nobody can check is a rumour.
    doeScoreNote:
      "The score out of 100 is kits born alive per mating — fertility × average litter size — measured against \"every mating produces a litter of your farm's average size\" = 100. Deliberately hard, so most good does sit below it. A doe without enough record yet reads «—», not zero.",

    topSectionTitle: "Best-performing does",
    topEmpty: "No doe has enough record to rank yet. The score needs matings and kindlings on file.",
    topCountLabel: "Does listed",
    topAvgScoreLabel: "Farm average score",
    topBestScoreLabel: "Best score",
    topColRank: "Rank",
    // Says what the list is FOR. A leaderboard nobody acts on is decoration:
    // the point is that the next generation comes out of these cages.
    topNote: (sharePct: number) =>
      `The best ${sharePct}% of your does by score — the half your replacements should come out of. A share rather than a fixed count, because "the top twenty" means most of a 30-doe barn and the top 5% of a 400-doe one. Equal scores go to the doe with more kindlings, because a number with eight litters behind it is worth more than the same number with three. A doe without enough record is not ranked: one excellent litter makes a lucky doe, not an excellent one.`,
  },
  records: {
    title: "Records",
    description: "All farm operation logs in one place.",
    // No "Log" suffix here — the page title already says Records and the
    // heading under the bar repeats the full name.
    tabMating: "Mating",
    tabPregnancyTest: "Pregnancy Test",
    tabResorption: "Resorption",
    tabKindling: "Kindling",
    tabWeaning: "Weaning",
    tabFostering: "Fostering",
    tabMortality: "Mortality",
    tabCulling: "Culling",
    fromLabel: "From",
    toLabel: "To",
    applyButton: "Filter",
    rangeMonthButton: "Month",
    rangeQuarterButton: "3 months",
    rangeYearButton: "Year",
    rangeAllButton: "Since day one",
  },
  dailyOperations: {
    title: "Daily Operations",
    description: "Manage and track all daily farm operations in one place.",
    tabMating: "Mating",
    tabPregnancyTest: "Pregnancy Test",
    tabKindling: "Kindling",
    tabWeaning: "Weaning",
    tabFostering: "Fostering",
  },
  supportOperations: {
    title: "Support Operations",
    description: "Manage nest box installations and mortality tracking in one place.",
    tabNestBox: "Nest Box Installation",
    tabMortality: "Mortality Tracking",
  },
  dailyRounds: {
    title: "Doe & Buck Rounds",
    description: "Daily farm rounds for does and bucks to record actions in real-time.",
    tabDoesRounds: "Does Rounds",
    tabBucksRounds: "Bucks Rounds",
  },
  herdAndStock: {
    title: "Flock & Stock",
    description: "Manage breeds, mothers, bucks, and herd records in one place.",
    tabStock: "Breeds & Stock",
    tabMothers: "Mothers",
    tabBucks: "Bucks",
  },
  finance: {
    title: "Finance",
    description: "Income, expenses, and profit / loss.",
    rangeMonth: "This month",
    rangeYear: "This year",
    rangeAll: "All time",
    incomeLabel: "Income",
    expenseLabel: "Expenses",
    netLabel: "Net profit / loss",
    transactionsTitle: "Transactions",
    emptyTitle: "No transactions in this range",
    emptyDescription: "Add income or expenses using the form above.",
    farmWideSuffix: " · Farm-wide",
    addedToast: "Transaction added",
    dateLabel: "Date",
    typeLabel: "Type",
    categoryLabel: "Category",
    feedTonsLabel: "Quantity (tons)",
    feedTonsHint: (price: string) => `Fills the amount at ${price} per ton.`,
    amountLabel: (currency: string) => `Amount (${currency})`,
    rabbitLabel: "Rabbit (optional)",
    farmWideOption: "Farm-wide",
    notesLabel: "Notes",
    submitButton: "Add transaction",
    deletedToast: "Transaction deleted",
    // Recurring fixed monthly expenses
    recurringTitle: "Fixed monthly expenses",
    recurringDescription:
      "Rent, salaries, electricity — costs that arrive whether the farm produced anything or not. Set them up once here and press “Post what's due” instead of typing them in every month.",
    recurringMonthlyTotal: (amount: string) => `Monthly total: ${amount}`,
    recurringPerDoeHint: (amount: string) => `That is ${amount} per doe per month.`,
    recurringEmpty: "No fixed expenses set up yet.",
    recurringDayLabel: "Day due",
    recurringDayHint: "1–28, so a short month can never pass unbooked.",
    recurringStartLabel: "Starting from",
    recurringStartHint: "No month before this date is ever posted.",
    recurringDayOf: (day: number) => `Day ${day} of each month`,
    recurringAddButton: "Add fixed expense",
    recurringAddedToast: "Fixed expense added",
    recurringRemovedToast: "Fixed expense removed",
    recurringRemoveConfirm:
      "Removing this template only stops future months — transactions already posted stay in the ledger. Continue?",
    recurringPostButton: "Post what's due",
    // Entries, not months — three templates over three unbooked months is nine
    // rows, and "9 months" would read as three years behind.
    recurringDueCount: (count: number, amount: string) =>
      count === 1 ? `1 entry due (${amount})` : `${count} entries due (${amount})`,
    recurringNothingDue: "Nothing due right now — every past month is already posted.",
    recurringPostedToast: (count: number) =>
      count === 1 ? "1 transaction posted" : `${count} transactions posted`,
    recurringPostedNote: "Fixed monthly expense",
  },
  rabbits: {
    saveButton: "Save rabbit",
    tagLabel: "Rabbit #",
    tagPlaceholder: "e.g. 42 or 5A",
    weightLabel: "Weight (kg)",
    weightPlaceholder: "kg",
    doeCardTitle: "Doe card",
    buckCardTitle: "Buck card",
    rabbitCardTitle: "Rabbit card",
    editDoeCardButton: "Edit doe card",
    editBuckCardButton: "Edit buck card",
    editRabbitCardButton: "Edit rabbit card",
    breedLabel: "Breed",
    breedPlaceholder: "Choose a breed…",
    colorLabel: "Color",
    colorPlaceholder: "e.g. White",
    sexLabel: "Sex",
    statusLabel: "Status",
    dobLabel: "Date of birth",
    cageLabel: "Cage / pen",
    cagePlaceholder: "e.g. A1",
    sireLabel: "Sire",
    sireNone: "Unknown / none",
    sirePlaceholder: "Choose a buck…",
    damLabel: "Dam",
    damNone: "Unknown / none",
    damPlaceholder: "Choose a doe…",
    acquiredDateLabel: "Acquired date",
    acquiredFromLabel: "Acquired from",
    acquiredFromPlaceholder: "e.g. Maple Ridge Farm",
    photoUrlLabel: "Photo URL",
    notesLabel: "Notes",
    cancelButton: "Cancel",
    newPageBack: "Back to litter",
    newPageTitle: "Tag kit",
    newPageDescription: "Promote a kit from this litter to a full rabbit record.",
    createButton: "Create rabbit",
    editPageBack: "Back to rabbit",
    editPageTitleTagged: (tag: string) => `Edit #${tag}`,
    editPageTitleUntagged: "Edit unnumbered juvenile",
    editPageDescription: "Update this rabbit's details.",
    saveChangesButton: "Save changes",
    detailBackToStock: "Back to juveniles",
    // A rabbit's card is linked from its fertility report only, so back lands
    // there rather than on the does/bucks board.
    detailBackToDoesFertility: "Back to does fertility",
    detailBackToBucksFertility: "Back to bucks fertility",
    // Except on the edit screen: its only entry point is the تعديل button in
    // the does/bucks table, so back lands on the table it came from.
    detailBackToMothers: "Back to does",
    detailBackToBucks: "Back to bucks",
    untaggedAlt: "Unnumbered juvenile",
    taggedTitle: (tag: string) => `#${tag}`,
    untaggedTitle: "Unnumbered juvenile",
    editButton: "Edit",
    breedingHistoryHeading: "Breeding history",
    changeStatusButton: "Change status",
    statusSetToast: (statusLabel: string) => `Status set: ${statusLabel}`,
    currentSuffix: " (current)",
    historyEmptyTitle: "No breeding history yet",
    doeHistoryEmptyDescription:
      "Any mating for this doe will show up here with all its stages: testing, kindling, and weaning.",
    buckHistoryEmptyDescription:
      "Any mating for this buck will show up here with the test result and litter size.",
    colIndex: "#",
    colMatingDate: "Mating date",
    colBuckTag: "Buck #",
    colTestDate: "Test date",
    colTestResult: "Test result",
    colKindlingDate: "Kindling date",
    // Born = alive at the kindling moment, frozen. The Alive/Dead pair after it
    // is the nursing count, i.e. after fostering and kit deaths.
    colBornAtKindling: "Born",
    // Banner cell spanning Alive + Dead, so the pair reads as nursing counts.
    groupNursing: "Nursing",
    colBornAlive: "Alive",
    colBornDead: "Dead",
    colWeaningDate: "Weaning date",
    colWeanedCount: "Weaned",
    colDoeTag: "Doe #",
    colDoeBreed: "Breed",
    colDoeState: "Doe state",
    colBornCount: "Litter size",
    bornWithDead: (alive: number, dead: number) => `${alive} (+${dead} dead)`,
    fertilityStatsHeading: "Fertility stats",
    fertilityRateLabel: "Fertility rate",
    avgLitterSizeLabel: "Avg. litter size",
    joinedHerdLabel: "Joined herd",
    joinedHerdLabelHerd: "Joined herd pen",
    totalMatingsLabel: "Total matings",
    totalKindlingsLabel: "Total kindlings",
    avgWeanedLabel: "Avg. weaned",
    weaningRetentionLabel: "Weaning retention",
    tagInUse: "This rabbit number is already in use",
    motherTagInUse: "This doe number is already in use",
    buckTagInUse: "This buck number is already in use",
    selfParentError: "A rabbit cannot be its own parent",
    deleteBlockedByBreeding: "This rabbit can't be deleted because it's linked to a breeding record",
    strainNotFound: "Juvenile not found",
    cageRequiredFirst: "Record the cage number first",
    weightRequiredFirst: "Record the weight first",
    invalidCageFallback: "Invalid cage number",
    invalidWeightFallback: "Invalid weight",
  },
  breedings: {
    matingLabel: "Buck (sire)",
    buckPlaceholder: "Choose a buck…",
    doeLabel: "Doe (dam)",
    doePlaceholder: "Choose a doe…",
    matingDateLabel: "Mating date",
    matingDateHint: (days: number) =>
      `Expected kindling date is calculated automatically as mating date + ${days} days.`,
    outcomeLabel: "Outcome",
    actualKindlingDateLabel: "Actual kindling date",
    actualKindlingDateHint: "Leave blank until she kindles.",
    notesLabel: "Notes",
    saveButton: "Save mating",
    cancelButton: "Cancel",
    editPageTitle: "Edit mating",
    saveChangesButton: "Save changes",
    editPageBack: "Back to mating",
    detailBackToDoe: "Back to doe",
    changeOutcomeButton: "Set outcome",
    outcomeSetToast: (outcomeLabel: string) => `Outcome set: ${outcomeLabel}`,
    currentSuffix: " (current)",
    editButton: "Edit",
    colMatingDate: "Mating date",
    colExpectedKindling: "Expected kindling",
    colActualKindling: "Actual kindling date",
    colLitter: "Litter",
    inDays: (days: number) => `in ${days} day${days === 1 ? "" : "s"}`,
    overdueDays: (days: number) => `${days} day${days === 1 ? "" : "s"} overdue`,
    viewLitter: "View litter",
    notRecorded: "Not recorded",
    notesHeading: "Notes",
    litterCardTitle: "Litter",
    colBornAlive: "Born alive",
    colBornDead: "Stillborn",
    colWeaned: "Weaned",
    openLitterButton: "Open litter",
    recordKindlingCardTitle: "Record kindling",
    kindlingDateLabel: "Kindling date",
    bornAliveLabel: "Born alive",
    bornDeadLabel: "Stillborn",
    weanedLabel: "Weaned (optional)",
    weanedPlaceholder: "Leave blank until weaning",
    weaningDateLabel: "Weaning date (optional)",
    recordKindlingButton: "Record kindling",
    notFound: "Breeding not found",
    litterAlreadyExists: "A litter is already recorded for this mating.",
    doeNotFound: (tag: string) => `No doe with #${tag}`,
    noCurrentLitter: (tag: string) => `Doe #${tag} has no current nursing litter`,
    notEnoughBornAlive: (tag: string, count: number) => `Doe ${tag} only has ${count} alive`,
    transferSuccessToast: "Fostering transfer recorded",
    weanedExceedsBornAlive: "Weaned can't exceed the number born alive",
    noLitterForBreeding: "No litter recorded for this breeding — record the kindling first",
    blankReference: "This row isn't linked to a record — refresh and try again",
    noNursingKitsToRecordDeath: "No live kits to record a death for",
  },
  litters: {
    notFound: "Litter not found",
    editPageTitle: "Edit litter",
    editPageBack: "Back to litter",
    kindlingDateLabel: "Kindling date",
    bornAliveLabel: "Born alive",
    bornDeadLabel: "Stillborn",
    weanedLabel: "Weaned",
    weaningWeightLabel: "Weaning weight (g)",
    weaningDateLabel: "Weaning date",
    notesLabel: "Notes",
    saveChangesButton: "Save changes",
    cancelButton: "Cancel",
    detailBackToBreeding: "Back to mating",
    pageTitle: "Litter",
    editLitterButton: "Edit litter",
    colKindlingDate: "Kindling date",
    colTotalBorn: "Total born",
    colBornAlive: "Born alive",
    colBornDead: "Stillborn",
    colWeaned: "Weaned",
    colWeaningDate: "Weaning date",
    colSurvivalRate: "Survival rate",
    colBreeding: "Mating",
    viewBreeding: "View mating",
    taggedKitsHeading: (count: number) => `Tagged kits (${count})`,
    tagKitButton: "Tag kit",
    noKitsTagged:
      'No kits tagged yet. Use "Tag kit" to promote one to a full rabbit record (parents are filled in automatically from this litter).',
    kitTag: (tag: string) => `#${tag}`,
  },
  doesFertility: {
    title: "Does Fertility Report",
    description: (count: number) => `Fertility rates, births, matings, and weaning survival rates for each doe in the herd (${count} does).`,
    colDoeTag: "Doe #",
    colBreed: "Breed",
    colStatus: "Status",
    colDoeState: "Reproductive State",
    colBreedings: "Total Breedings",
    colKindlings: "Total Births",
    colFertilityRate: "Fertility Rate",
    // True litter size: averaged from the frozen KindlingLog.bornAliveAtKindling,
    // so fostering and kit deaths can't move it.
    colAvgBornAtKindling: "Avg Born Alive",
    // "Nursed", not "born": this report sums KindlingLog.bornAlive, the count
    // after fostering and kit deaths. The frozen birth count lives on the doe page.
    colAvgBorn: "Avg Nursed",
    colAvgWeaned: "Avg Weaned Kits",
    colAvgWeaningWeight: "Avg Weaning Weight (g)",
    colWeaningSurvivalRate: "Weaning Survival",
    emptyTitle: "No does registered",
    emptyDescription: "Register active does with a tag number to see them here.",
    statsHeading: "Average Doe Fertility",
    statAvgBornAtKindling: "Avg Born Alive",
    statTotalKindlings: "Total Births",
    statTotalBreedings: "Total Breedings",
    statWeaningSurvival: "Weaning Survival",
    statAvgWeaned: "Avg Weaned Kits",
    // Same number as the column, farm-wide — must keep the same name.
    statAvgBorn: "Avg Nursed",
  },
  bucksFertility: {
    title: "Bucks Fertility Report",
    description: (count: number) => `Fertility rates, successful matings, and total offspring sired for each buck in the herd (${count} bucks).`,
    colBuckTag: "Buck #",
    colBreed: "Breed",
    colStatus: "Status",
    colBreedings: "Total Breedings",
    // Numerator of a buck's fertility rate: positive palpations, not kindlings —
    // a pregnancy lost after confirmation is the doe's outcome, not his.
    colPregnancies: "Confirmed Pregnancies",
    colFertilityRate: "Fertility Rate",
    colAvgBorn: "Avg Litter Size",
    colTotalBorn: "Total Offspring",
    emptyTitle: "No bucks registered",
    emptyDescription: "Register active bucks with a tag number to see them here.",
    statsHeading: "Average Buck Fertility",
    statPregnancies: "Confirmed Pregnancies",
    statTotalBreedings: "Total Breedings",
    statTotalBorn: "Total Offspring",
    statAvgBorn: "Avg Litter Size",
  },
};
